...
class Level_01(Level):
    ...
    def __init__(self, player):
        ...
        # Array with width, height, x, y, and image path of platform
        level = [
            [210, 70, 500, 500, "./Images/goomba.png"],
            [210, 70, 200, 400, "./Images/goomba.png"],
            [210, 70, 600, 300, "./Images/goomba.png"],
        ]

        # Go through the array above and add platforms
        for platform in level:
            block = Platform(platform[4])  # Load image from the provided path
            block.rect.x = platform[2]
            block.rect.y = platform[3]
            block.player = self.player
            self.platform_list.add(block)
